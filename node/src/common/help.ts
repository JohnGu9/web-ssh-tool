...
export function help() {
  ...
  table({
    ...
    '-h --home': new Description({ description: 'setup file explorer default path', example: '--home=/home' }),
    '-l --log': new Description({ description: 'setup log file output dest', example: '--log=/etc/my_server.log' }),
  ...
  log();
  log(`tip: config file ("-c" or "--config") also support env variable            (example: ${configEnvVariable}=/etc/config.json && ./web-ssh-tool)`);
  log();
}
