...
export function SharedAxisTransition(props: SharedAxisTransition.Props) {
  return (
    <SwitchTransition className={props.className} id={props.id} style={props.style}
...
export class SwitchTransition extends React.Component<SwitchTransition.Props, SwitchTransition.State> {
  ...
  componentDidUpdate(oldProps: SwitchTransition.Props) {
    if (oldProps.id !== this.props.id) {
      ...
      this._pipeLine.post(async () => {
        ...
        this.setState({ style: { pointerEvents: 'none', ...outAnimation.style } });
...
export namespace SwitchTransition {
  export type Props = {
    className?: string;
    style?: React.CSSProperties,
    children: React.ReactNode,
