...
export const ThemeContext = React.createContext<ThemeService.State>({ themeData: themes.lightTheme, isDark: false });
...
export class ThemeService extends React.Component<ThemeService.Props, ThemeService.State> {
  constructor(props: ThemeService.Props) {
    ...
    this.state = { themeData: this.isDark ? themes.darkTheme : themes.lightTheme, isDark: this.isDark }
  }
  ...
  _listener(event: MediaQueryListEvent) {
    this.setState({
      themeData: this.isDark ? themes.darkTheme : themes.lightTheme,
      isDark: this.isDark
    });
  }
...
namespace ThemeService {
  ...
  export type State = { themeData: ThemeData, isDark: boolean }
}
